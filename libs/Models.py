...
class ElasticModel(BaseModel):
	def __init__(self, E, nu, du, v, dx):
		self.E = E
		self.nu = nu
		self.du = du
		self.v = v
	...
	def compute_matrix(self):
		lame = self.E*self.nu/((1+self.nu)*(1-2*self.nu))
		G = self.E/(2 +2*self.nu)
		x = 2
		C_array = np.array([
		...
		self.C = as_matrix(C_array)

	def compute_stress(self, stress, eps_e, TS):
		stress_form = voigt2stress(dot(self.C, strain2voigt(eps_e)))
		stress.assign(local_projection(stress_form, TS))

	def build_A(self):
		a_form = inner(sigma(self.C, epsilon(self.du)), epsilon(self.v))*self.dx
		self.A = assemble(a_form)

	def build_b(self):
		self.b = assemble(0*self.dx)
...
class CreepDislocation(BaseModel):
	def __init__(self, A, n, T, theta, model_e, du, v, dx):
		self.du = du
		self.v = v
		self.dx = dx
		self.theta = theta
	...
	def compute_creep_strain_rate(self, eps_cr_rate, stress, TS):
		...
		eps_cr_rate.assign(local_projection(self.B*(von_Mises**(self.n-1))*s, TS))

	def compute_creep_strain(self, eps_cr, eps_cr_old, eps_cr_rate_old, eps_cr_rate, dt, TS):
		eps_cr.assign(local_projection(eps_cr_old + dt*(self.theta*eps_cr_rate_old + (1 - self.theta)*eps_cr_rate), TS))
		# eps_cr.assign(eps_cr_old + dt*(self.theta*eps_cr_rate_old + (1 - self.theta)*eps_cr_rate))



class CreepPressureSolution(BaseModel):
...
class CreepDislocation2(BaseModel):
	def __init__(self, A, n, T, C, theta, du, v, dx, TS):
		self.du = du
		self.v = v
		self.dx = dx
		self.TS = TS
	...
	def build_b(self):
		b_form = inner(sigma(self.C, self.eps_cr), epsilon(self.v))*self.dx
		self.b = assemble(b_form)

	def compute_creep_strain_rate(self, stress):
		s = stress - (1./3)*tr(stress)*Identity(3)
		von_Mises = sqrt((3/2.)*inner(s, s))
	...
	def compute_creep_strain(self, stress, dt):
		...
		self.eps_cr.assign(self.eps_cr_old + dt*(self.theta*self.eps_cr_rate_old + (1-self.theta)*self.eps_cr_rate))


class VoigtElement(BaseModel):
	def __init__(self, A, n, E, nu, theta, du, v, dx, TS):
		self.du = du
		self.v = v
		self.dx = dx
		self.TS = TS
	...
	def initialize_tensors(self):
		...
		self.eps_v_rate = local_projection(zero_tensor, self.TS)
		self.eps_v_rate_old = local_projection(zero_tensor, self.TS)
		self.stress_v = local_projection(zero_tensor, self.TS)

	def update_internal_variables(self):
		self.eps_v_old.assign(self.eps_v)
		self.eps_v_rate_old.assign(self.eps_v_rate)

	def build_A(self):
		pass

	def build_b(self):
		b_form = inner(sigma(self.C, self.eps_v), epsilon(self.v))*self.dx
		self.b = assemble(b_form)

	def compute_viscous_stress(self, stress):
		stress_spring = voigt2stress(dot(self.C, strain2voigt(self.eps_v)))
		self.stress_v.assign(local_projection(stress - stress_spring, self.TS))

	# def compute_viscous_strain_rate(self, stress):
	# 	self.compute_viscous_stress(stress)
	# 	s = self.stress_v - (1./3)*tr(self.stress_v)*Identity(3)
	# 	von_Mises = sqrt((3/2.)*inner(s, s))
	# 	self.eps_v_rate.assign(local_projection(self.A*(von_Mises**(self.n-1))*s, self.TS))

	def compute_viscous_strain_rate(self, stress):
		self.compute_viscous_stress(stress)
		self.eps_v_rate.assign(local_projection(self.A*self.stress_v, self.TS))

	def compute_viscous_strain(self, stress, dt):
		self.compute_viscous_strain_rate(stress)
		self.eps_v.assign(self.eps_v_old + dt*(self.theta*self.eps_v_rate_old + (1-self.theta)*self.eps_v_rate))
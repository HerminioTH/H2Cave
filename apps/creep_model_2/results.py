...
def plot_num(axis, output_folder):
	...
	eps_vp = pd.read_excel(os.path.join(output_folder, "avg", "eps_vp.xlsx"), index_col=0)
	axis[0,3].plot(eps_vp["Time"][::step]/hour, 100*(eps_vp["22"][::step]), ".-", color="0.15", label="Axial strain", ms=8, mfc="steelblue")
	axis[0,3].plot(eps_vp["Time"][::step]/hour, 100*(eps_vp["00"][::step]), ".-", color="0.15", label="Radial strain", ms=8, mfc="lightcoral")
	axis[0,3].set_xlabel("Time [hour]", size=12, fontname="serif")
	axis[0,3].set_ylabel("Visco-plastic strain [%]", size=12, fontname="serif")
	axis[0,3].grid(True)
	axis[0,3].legend(loc=0, shadow=True, fancybox=True, ncol=1)
	...
	axis[1,2].plot(alpha["Time"][::step]/hour, alpha["Scalar"][::step], ".-", color="0.15", label=label_name, ms=8, mfc="steelblue")
	axis[1,2].set_xlabel("Time [hour]", size=12, fontname="serif")
	axis[1,2].set_ylabel(r"$\alpha$", size=12, fontname="serif")
	axis[1,2].grid(True)
	...
	axis[1,3].plot(Fvp["Time"][::step]/hour, Fvp["Scalar"][::step], ".-", color="0.15", label=label_name, ms=8, mfc="steelblue")
	axis[1,3].set_xlabel("Time [hour]", size=12, fontname="serif")
	axis[1,3].set_ylabel(r"$F_{vp}$", size=12, fontname="serif")
	axis[1,3].grid(True)
	axis[1,3].legend(loc=0, shadow=True, fancybox=True, ncol=1)

...
def main():
	...
	# for time_index in range(0, len(t_day), 3):
		...
		ax1.plot(x, z, ".", color="white", label="Initial shape")
		ax1.plot(x+dx, z+dz, ".", color="0.7", label="Current shape")
		...
		ax1.plot([x[index_C], x[index_C]+beta*u[time_index,index_C]], [z[index_C], z[index_C]+beta*w[time_index,index_C]], ".-", color="gold")
		ax1.plot([x[index_D], x[index_D]+beta*u[time_index,index_D]], [z[index_D], z[index_D]+beta*w[time_index,index_D]], ".-", color="lightgreen")

		ax1.text(-24, -200, "A", size=12, fontname="serif", color="lightcoral")
		ax1.text(+24, -245, "B", size=12, fontname="serif", color="steelblue")
		ax1.text(+24, -415, "C", size=12, fontname="serif", color="gold")
		ax1.text(-24, -470, "D", size=12, fontname="serif", color="lightgreen")
		...
		ax1.text(108, -475, f"*Amplification factor: {beta}x", size=10, fontname="serif", color="0.7")
		...
		ax2.plot(t_day, s(u, v, w, index_C), ".-", color="gold", label="Point C")
		ax2.plot(t_day, s(u, v, w, index_D), ".-", color="lightgreen", label="Point D")
		ax2.grid(True)
		ax2.set_xlim(-32/24, 552/24)
		ax2.set_ylim(-0.036, 0.43)
		ax2.plot([t_day[time_index], t_day[time_index]], [-0.001, 0.43], "-", color="white", linewidth=1.0)
		ax2.text(t_day[time_index]-1, -0.018, "t=%.1f days"%(t_day[time_index]), color="white", size=10, fontname="serif")
